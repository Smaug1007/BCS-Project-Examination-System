...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2) {
  ...
  if ($ans == $ansid) {
    ...
    $w++;
    //$s = $s - $wrong;    write this if you want negative marking 
    $q = mysqli_query($con, "UPDATE `history` SET `score`=$s,`level`=$sn,`wrong`=$w, date=NOW() WHERE  email = '$email' AND eid = '$eid'") or die('Error147');
  }
  if ($sn != $total) {
    $sn++;
    header("location:landingpage.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total") or die('Error152');

  } else if ($_SESSION['key'] != 'nasir123') {
    ...
    header("location:landingpage.php?q=result&eid=$eid");
  } else {
    header("location:landingpage.php?q=result&eid=$eid");
  }
}
...
if (@$_GET['q'] == 'quizre' && @$_GET['step'] == 25) {
  ...
  $sun = $sun - $s;
  $q = mysqli_query($con, "UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'") or die('Error174');
  header("location:landingpage.php?q=quiz&step=2&eid=$eid&n=1&t=$t");
}
